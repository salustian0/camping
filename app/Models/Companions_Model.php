...
class Companions_Model extends Model{
    //Definições
	...
	public function listing($where = array(), $limit = array()){
		$this->select("{$this->table}.*,
						...
						users.name as responsible,users.docNumber as responsible_docNumber", FALSE);
		$this->where("{$this->table}.active"	, 'Y');
		$this->join("users", "users.id = {$this->table}.idUserFk");
		$this->where($where);
		//$this->orderBy("{$this->table}.name"	, 'ASC');
	...
	public function countPaginate($where = array()){
		...
		$this->where("{$this->table}.active"	, 'Y');
		$this->join("users", "users.id = {$this->table}.idUserFk", "LEFT");
		$this->where($where);
